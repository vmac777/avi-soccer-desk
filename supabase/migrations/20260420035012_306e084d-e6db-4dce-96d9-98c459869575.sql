CREATE OR REPLACE FUNCTION public.submit_news_item(
  p_url text,
  p_blurb text,
  p_urgency text,
  p_club_ids uuid[]
...
BEGIN
  ...
  IF v_role NOT IN ('admin', 'intern') THEN
...
REVOKE ALL ON FUNCTION public.submit_news_item(text, text, text, uuid[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_news_item(text, text, text, uuid[]) TO authenticated;